...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView {
    ...
    return [_sectionPhotos count];
}

- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    // Return the number of rows in the section.
    NSDictionary *sectionItem = [_sectionPhotos objectAtIndex:(int)section];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString *path = [[NSBundle mainBundle] pathForResource:[item objectForKey:@"image"] ofType:nil];
    cell.backgroundView = [[UIImageView alloc] initWithImage:[UIImage imageWithContentsOfFile:path]];
...
- (void)reloadTableView:(NSNotification *)noti {
    ...
    if (![newMode isEqualToString:@"keep"])
        _mode = newMode;
...
- (void)makeSectionWithAction:(NSString *)action {
    _sectionPhotos = [[NSMutableArray alloc] init];
    ...
    if ([action isEqualToString:@"sort"]) {
        ...
        NSArray *sortYears = [[yearDic allKeys] sortedArrayUsingSelector:@selector(caseInsensitiveCompare:)];
        for (NSString *year in sortYears) {
            ...
            [_sectionPhotos addObject:section];
        }
    }
    else {
...
- (NSDictionary *)itemForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    for (int i = 0; i < indexPath.section; i++) {
        NSDictionary *sectionItem = [_sectionPhotos objectAtIndex:(int)i];
        NSNumber *sectionCount = [sectionItem objectForKey:@"count"];
